...
object MovieLensALS {
  ...
  def main(args:Array[String]) {
    ...
    val ratings = sc.textFile(movielensHomeDir + "/reco.txt").map {
    ...
    val movies = sc.textFile(movielensHomeDir + "/reco.txt").map {
    ...
    val myUserIds = myRatings.map(_.user).toSet

    val writer = new PrintWriter(new File(movielensHomeDir + "fut.txt"))
    ...
    for(id <- myUserIds) {
        ...
        recommendations.foreach { r =>
          ...
          writer.write("%d".format(id) + "\t" + r.product + "\n")	
          println("%d".format(id) + "\t" + r.product)
